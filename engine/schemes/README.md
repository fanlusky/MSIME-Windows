...
5. **词库** — 新方案如果需要自己的表，表结构受 [`../contracts/dictionary/format.json`](../contracts/dictionary/format.json) 约束。建库流水线不在本仓库内（见 [`../UPSTREAM.md`](../UPSTREAM.md)），词库是 `product-lock.json` 钉住的发布产物，所以**新表要先有人出一版带这张表的词库发布**，再把锁指过去；在那之前方案只能落在已有的表上。数据的再分发授权要求见 [`../NOTICE.md`](../NOTICE.md)。
